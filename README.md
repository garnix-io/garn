...
All `garn.ts` files require the first import, and most often you also need the
second. Besides that, you almost always need nothing else.
...
**garn** then generates a `flake.nix` that, for every exported variables of the `garn.ts`,
...
you can write pure Nix expressions in **garn**, and import Nix projects, but
...
- **Nix**: Nix is a brilliant idea with an immense amount of volunteer effort
  behind it, and the giant shoulders on which **garn** stands. It has a rather
  ...
  stack. For complex projects, you're almost always better off picking Nix.
  ...
  or binary: **garn** it both allows including
  Nix expressions or dependencies within it, and generates a `flake.nix` that
