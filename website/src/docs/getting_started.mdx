...
nix profile install github:garnix-io/garn/v0.0.16
```
...
<Garn /> provides an `edit` command that downloads and starts a VSCodium configured with Deno LSP. It is started with it's own configuration and data directories, so as to not clobber any configuration you may already have.
...
- [typescript api](https://doc.deno.land/https://garn.io/ts/v0.0.16/mod.ts)
