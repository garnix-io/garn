...
In the same directory you can now run `garn enter frontend` to drop you in
a new shell with `node` version 18 available.

Note that -- if you do this in a `git` repo -- `garn` requires that your source
files must be tracked by git. You have to at least add them with
...
environment. These `Check`s are run in a build sandbox. The downside of
sandboxing is that these checks won't have access to the internet. The upside
is that they'll be (almost) perfectly reproducible.

## Language agnostic

You can have multiple sub-projects with different tech stacks in a single
...
This enables anyone on your team to run, test, and develop any part of your
codebase, without having to install any language-specific tools and learn how
to use them.
...
CI on [garnix.io](https://garnix.io). See [the docs](https://garnix.io/docs)
for more info.
