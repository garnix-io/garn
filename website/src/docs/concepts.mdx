...
reproducible automated tests or other checks. They are being run in a sandbox,
...
addition they will have access to a _copy_ of your source files, that is
provided in the sandbox they are run in. That gives you the ability to define
...
`Package`s are instructons to `garn` about how to _build_ a set of files. For
...
Here's an example of a `Project` that contains a few `Check`s and `Package`s:
